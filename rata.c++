		...
		if (floor && !take_damage) {
			if (hurting < 12) {
				...
				if (key[sf::Key::A] && !key[sf::Key::D]) {  // Left
					...
					stop:
					if (xvelrel(floor) > 0) {
						//mutual_impulse(floor, MIN(ground_decel()*mass(), xvelrel(floor)), 0);
						add_vel(-MIN(ground_decel(), xvelrel(floor)), 0);
					}
					else if (xvelrel(floor) < 0) {
						//mutual_impulse(floor, MAX(-ground_decel()*mass(), xvelrel(floor)), 0);
						add_vel(-MAX(-ground_decel(), xvelrel(floor)), 0);
					}
				}
				 // Jump
