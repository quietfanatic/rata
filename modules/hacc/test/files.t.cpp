...
tap::Tester hacc_files_tester ("hacc-files", [](){
    ...
    doesnt_throw([](){string_to_file(test3, "test_file", APPEND);}, "Trying to append doesn't cause an error.");
    is(file_to_string("test_file"), test2 + test3, "Appending to a file works.");
    throws<hacc::Error>([](){string_to_file(test3, "test_file", CREATE);}, "Throw an error when mode is CREATE and the file exists.");
